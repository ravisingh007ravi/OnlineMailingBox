...
let getPlan = async function (req, res) {

    try {

        let getPlan = await planModel.find();
        
        if(getPlan.length==0){ return res.status(400).send({status:"false, message:NoData"})}
        res.status(201).send({ status: true, msg: "Get All Plan successfully", data: getPlan })
